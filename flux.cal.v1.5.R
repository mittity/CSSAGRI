...
#--- set the date for analysis 
 ...
 #  site.name = "MPI"
  ...
  ini.stamp = "2023-04-01 00:00:00"
  end.stamp = "2023-04-10 24:00:00"
  crop_id = "S10"
#  ndays=60
#  n30=48*ndays  
...
# date.id <- format(seq(as.POSIXct("2019-03-14 12:00:00", tz=""), length.out=10, by='15 min'),'%Y_%j %H:%M')
  ...
  if(ld.go) {
     ...
     #check file exist 
     wrk.yr = date.yyyy[i]
	     file.name <- paste(ec150.dir,"/",wrk.yr,"/",site.name,"_",date.yyyy[i],"_",date.mm[i],"_",date.dd[i],"_",
	   ...
	   #		       substr(date.hhmmss[i],start=1,stop=2),substr(date.hhmmss[i],start=4,stop=5),".dat",sep="")
	     ...
	     if(file.exists(file.name)==TRUE) {
             raw.data <- fun_load_ec150(
				dir.name=ec.dir, site.name=site.name,
    ...
    raw.data$CH4_mole_fraction <- as.numeric(raw.data$CH4_mole_fraction)
  ...
  #  T(j)=T(j)*(1+0.514*(Rhov(j)/Rhoa(j)))          !-----Change Sonic Temp to Air Temp
    ...
    # NO AIR PRESSURE DATA USE 100.0 Kpa  
     ...
     ch4.mole.f <- mean(raw.data$CH4_mole_fraction, na.rm=T) #[umole/mole]
