...
#load source file for the functions used in the program
  ...
  source("fun_load_gradient_20rice.R")
...
# search tildas files 
  tildas.dir   <- c("/work/ychen/CSSAGRI/EC_Raw/TILDAS_m/")
  ec150.dir    <- c("/work/ychen/CSSAGRI/EC_Raw/EC150_10min/")
...
# skip the non zero start and shift to next time 10 min 
# ceiling the initial minumtes to every 10min
  ini.mm <- ceiling(ini.mm/10)*10
...
# switch 00:00 to 24:00 
  ...
  n10  <- (fin.hh-ini.hh)*6 + floor(fin.mm/10) - 2    
...
# forward one step for none zero seconds start
  if(ini.ss!= 0) {ini.stamp <- as.POSIXct(ini.stamp, tz="") + 60*10}
  ini.stamp <- as.POSIXct(ini.stamp, tz="") + 60*10

  print( paste("EC150 starting date:",ini.hh,":",ini.mm,":",ini.ss, 
  	     "Total 10min steps:",n10, "For",target,"Filed!", sep=" ") )  
  date.id   <- format(seq(as.POSIXct(ini.stamp, tz=""), length.out=n10, by='10 min'),'%Y-%m-%d %H:%M:%S')
# date.id <- format(seq(as.POSIXct("2019-03-14 12:00:00", tz=""), length.out=10, by='10 min'),'%Y_%j %H:%M')
  ...
  try( gd.raw <- fun_load_gradient_20rice(filename=paste(gradient.dir,"/",gd_filename,sep=""), ld_plot=TRUE) )
  ...
  if(ld.go) {
     ...
     #check file exist 
	     ...
	     raw.data <- fun_load_ec150(dir.name=ec150.dir, prefix="CSSAGRI_",
             ...
             #if loading file gets error skip to next 10 mins 
...
write.table(file=paste("./plot_pdf/flux.table.0421_25.dat",sep=""),flux.table, row.names=FALSE, sep=",")
#
#
