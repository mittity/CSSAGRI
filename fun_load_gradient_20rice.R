...
fun_load_gradient_20rice <- function(filename="/work/ychen/CSSAGRI/GD_Raw/daily/20190714.xlsx",
                              ld_plot=FALSE,ld_spike=FALSE) {
#ld_plot=TRUE;ld_spike=FALSE
#filename="/work/ychen/CSSAGRI/GD_Raw/20190905.xls"
...
#print(paste("n15:",nn))
date.id   <- format(seq(as.POSIXct(ini.date, tz="UTC"), length.out=n10, by='10 min'),'%Y-%m-%d %H:%M:%S')
#
...
# 
   tmp.no.dn <- median( gd.raw$'NO(ppb)_30cm_88PY'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="30cm")], na.rm=TRUE) 
   tmp.no.up <- median( gd.raw$'NO(ppb)_200cm_88P'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="150cm")], na.rm=TRUE) 
#   
   tmp.nox.dn <- median( gd.raw$'NOx(ppb)_Station_780TR'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="30cm")], na.rm=TRUE) 
   tmp.nox.up <- median( gd.raw$'NOx(ppb)_Station_780TR'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="150cm")], na.rm=TRUE) 
   
#   }else{
#   tmp.n2o.dn <- median( gd.raw$'N2O(ppm)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE) 
#   tmp.n2o.up <- median( gd.raw$'N2O(ppm)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
 
#   tmp.co2.dn <- median( gd.raw$'CO2(ppm)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE ) 
#   tmp.co2.up <- median( gd.raw$'CO2(ppm)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
 
#   tmp.ch4.dn <- median( gd.raw$'CH4(ppm)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE) 
#   tmp.ch4.up <- median( gd.raw$'CH4(ppm)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
   
#   tmp.h2o.dn <- median( gd.raw$'H2O(%)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE) 
#   tmp.h2o.up <- median( gd.raw$'H2O(%)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
   
#   tmp.nh3.dn <- median( gd.raw$'NH3(ppb)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE) 
#   tmp.nh3.up <- median( gd.raw$'NH3(ppb)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
 
#   tmp.no.dn <- median( gd.raw$'NO(ppb)_88PY'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE) 
#   tmp.no.up <- median( gd.raw$'NO(ppb)_88PY'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
   
#   tmp.nox.dn <- median( gd.raw$'NOx(ppb)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="60cm")], na.rm=TRUE) 
#   tmp.nox.up <- median( gd.raw$'NOx(ppb)'[(gd.raw$Datetime>date.id[i])&(gd.raw$Datetime<date.id[i+1])&(gd.raw$Elevation=="180cm")], na.rm=TRUE) 
...
#
#   print(paste("up:",up.tmp)   )
#   print(paste("down:",dn.tmp) )
}
